  ...
  return (
    <div className="min-h-screen bg-[#0A0B0F] text-white p-4 md:p-6">
      ...
      <div className="max-w-6xl mx-auto mb-6 flex items-center justify-between">
        <div>
          <p className="text-gray-400 text-sm">Overview the best & most successful wallets</p>
          <h1 className="text-xl font-medium">
            Realized <span className="text-emerald-400">PnL</span> Leaderboard
          </h1>
        </div>
        <Button
      onClick={isInitialCheckComplete ? (isWalletConnected ? () => setShowSocialsModal(true) : handleConnectWallet) : undefined}
      className="bg-[#9D5EF4] hover:bg-[#8B4FE3] text-white"
      disabled={!isInitialCheckComplete}
    >
      {!isInitialCheckComplete ? "Loading..." : (isWalletConnected ? (userSocials.length > 0 ? "Edit Socials" : "Add Socials") : "Connect Wallet")}
    </Button>
      </div>
      ...
      <div className="max-w-6xl mx-auto mb-16">
        <Tabs defaultValue="daily">
          <TabsList className="bg-transparent border border-gray-800">
            <TabsTrigger value="daily" className="data-[state=active]:bg-gray-800">
              Daily
            </TabsTrigger>
            ...
            <Card
              ...
              className={cn(
                ...
                trader.position === "center"
                  ? "z-20 w-[340px] transform scale-[1.03] -translate-y-1 translate-z-[30px] p-5 pb-12 pt-4 bg-[#0F1115] hover:opacity-100 hover:scale-[1.05] hover:translate-z-[40px] border-2 border-purple-500/30"
            ...
            >
              ...
              <div
                className={cn(
                  ...
                  trader.position === "center" ? "transform translateZ(20px) py-1" : "transform translateZ(0px)",
                )}
              >
                ...
                <h3 className="text-lg font-medium mb-1 text-white">{trader.name}</h3>
                <p className="text-gray-500 text-sm mb-4">{trader.walletAddress}</p>
                <div className="text-2xl font-bold text-emerald-400 mb-1">{trader.pnl} ≋</div>
                ...
                <div className="flex items-center justify-center gap-4">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-sm text-gray-400">{trader.greenTrades}</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <span className="w-2 h-2 bg-red-400 rounded-full" />
                    <span className="text-sm text-gray-400">{trader.redTrades}</span>
                  </div>
                </div>
              ...
              {trader.position === "center" && (
                <>
                  ...
                  {/* Updated left and right border gradients */}
                  <div className="absolute left-0 inset-y-0 w-12 bg-gradient-to-r from-purple-500/20 to-transparent pointer-events-none" />
                  <div className="absolute right-0 inset-y-0 w-12 bg-gradient-to-l from-purple-500/20 to-transparent pointer-events-none" />
                  {/* New inner border */}
                  <div className="absolute inset-[3px] border border-purple-500/10 rounded-lg pointer-events-none" />
                </>
              )}
            </Card>
          ))}
      ...
      <div className="max-w-6xl mx-auto space-y-2">
        {rankedTraders.map((trader, i) => (
          <div key={i} className="bg-[#0F1115] border border-gray-800 rounded-lg p-4 flex items-center">
            <div className="flex items-center gap-4 w-[50%]">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center justify-between">
                  <p className="text-gray-500 text-sm">{trader.walletAddress}</p>
